    ...
    try {
      ...
      if (selectedRace.stage === 'FINAL') {
        ...
        validEntries.forEach((entry, index) => {
          ...
          const safeLogData = JSON.parse(JSON.stringify({
      ...
      const safeUpdateData = JSON.parse(JSON.stringify({
        entries: finalizedEntries,
        status: "OFFICIAL", 
        updatedAt: new Date().toISOString()
      }));

      batch.update(doc(db, 'races', safeRaceId), safeUpdateData);
      await batch.commit();
  ...
  return (
    <div className="p-8 pb-32">
      ...
      <div className="flex flex-col xl:flex-row gap-8">
        ...
        {/* 👉 大幅強化的左側列表區塊 */}
        <div className="w-full xl:w-[40%] flex flex-col gap-6 h-[calc(100vh-200px)]">
          
          {/* 待處理賽事區塊 */}
          <div className="bg-gray-900 border border-amber-500/30 rounded-xl flex-1 flex flex-col overflow-hidden shadow-xl">
            <div className="p-5 border-b border-gray-800 bg-gray-900 z-10 shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
              {isLoading ? (
                <div className="text-center text-gray-500 py-10 animate-pulse text-lg">正在同步賽程...</div>
              ) : pendingRaces.length === 0 ? (
                <div className="text-center text-emerald-500 py-10 font-bold bg-emerald-500/10 rounded-xl border border-emerald-500/20 text-lg">
                  🎉 目前無待處理賽事。
                </div>
              ) : (
                <div className="space-y-4">
                  {pendingRaces.map(race => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-sm font-bold mb-2 flex items-center gap-2 text-amber-400">
                          {isField ? '【田項】' : '【徑項】'} 
                          {race.stage === 'FINAL' ? '🏆 決賽 (FINAL)' : `初賽 - 第 ${race.groupNo} 組`}
                          {race.eventId.includes('RELAY') && <span className="bg-blue-600 text-white px-2 py-0.5 rounded text-xs ml-auto">接力 x{scoringSettings.relayMultiplier}</span>}
                        </div>
                        <div className="text-2xl font-black text-white tracking-wide">{race.eventId}</div>
                      </button>
                    )
                  })}
                </div>
          ...
          {/* 已完成的賽事區塊 */}
          <div className="bg-gray-900 border border-gray-700 rounded-xl flex-1 flex flex-col overflow-hidden shadow-xl">
            <div className="p-5 border-b border-gray-800 bg-gray-900 z-10 shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
              {officialRaces.length === 0 ? (
                <div className="text-center text-gray-600 py-10 text-lg">尚無已公佈的成績。</div>
              ) : (
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-xl transition-all border-2 ${
                          selectedRace?.id === race.id 
                            ? 'bg-gray-700 border-gray-400 shadow-lg scale-[1.02]'  
                      ...
                      >
                        ...
                        <div className="text-xl font-bold text-gray-300 flex justify-between items-center">
                          {race.eventId}
                          <span className="text-sm text-blue-400 font-bold bg-blue-900/30 px-3 py-1 rounded-lg">修改 ✎</span>
                        </div>
                      </button>
                    )
                  })}
        ...
        <div className="w-full xl:w-[60%]">
          {!selectedRace ? (
             <div className="h-full flex flex-col items-center justify-center bg-gray-900/50 border-2 border-dashed border-gray-700 rounded-xl p-10 text-gray-500">
               <svg className="w-24 h-24 mb-6 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
               <h2 className="text-3xl font-bold">請從左側選擇一場賽事</h2>
             </div>
          ) : (
            <div className={`border-2 rounded-xl p-8 shadow-2xl bg-gray-900 flex flex-col ${selectedRace.stage === 'FINAL' ? 'border-purple-500/30' : 'border-amber-500/30'}`}>
              
              <div className="flex justify-between items-center mb-8 bg-gray-950 p-5 rounded-xl border border-gray-800 shadow-inner">
                <div>
                  ...
                  <h2 className="text-4xl font-black tracking-wide mt-1">{selectedRace.eventId}</h2>
              ...
              <div className="flex flex-col lg:flex-row gap-6 mb-8">
                ...
                <div className={`space-y-4 ${selectedRace.stage === 'FINAL' ? 'lg:w-2/3' : 'w-full'}`}>
                  {selectedRace.entries.map((entry) => (
                    <div 
                      key={entry.lane} 
                      className={`flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl border-2 transition-colors gap-4 ${
                    ...
                    >
                      <div className="flex items-center gap-4 w-full md:w-1/3">
                        <div className="w-12 h-12 rounded-xl bg-gray-900 border border-gray-700 flex flex-col items-center justify-center shrink-0">
                      ...
                      <div className="flex items-center gap-4 w-full md:w-2/3 justify-end">
                        
                        <div className="flex bg-gray-950 rounded-lg p-1 border border-gray-800 shrink-0">
                           <button onClick={() => handleStatusChange(entry.lane, 'VALID')} className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${entry.entryStatus === 'VALID' ? 'bg-blue-600 text-white' : 'text-gray-500'}`}>正常</button>
                          <button onClick={() => handleStatusChange(entry.lane, 'ABS')} className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${entry.entryStatus === 'ABS' ? 'bg-orange-600 text-white' : 'text-gray-500'}`}>ABS</button>
                          <button onClick={() => handleStatusChange(entry.lane, 'DQ')} className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${entry.entryStatus === 'DQ' ? 'bg-red-600 text-white' : 'text-gray-500'}`}>DQ</button>
                        </div>
                        ...
                        {selectedRace.isFieldEvent ? (
                            ...
                            {[0, 1, 2].map(attemptIdx => (
                              <div key={attemptIdx} className="relative">
                                <input 
                                  ...
                                  className="w-20 text-center text-lg font-bold font-mono py-3 px-2 rounded-lg bg-gray-950 border border-gray-700 text-white focus:border-emerald-500 focus:outline-none placeholder-gray-600"
                                />
                              </div>
                            ))}
                          ...
                          <div className="relative">
                            <input 
                              ...
                              disabled={entry.entryStatus !== 'VALID'}
                              className="w-36 text-right text-3xl font-black font-mono p-3 rounded-xl bg-gray-950 border border-gray-700 text-white focus:border-blue-500 focus:outline-none"
                            />
                            <span className="absolute right-4 bottom-2 text-sm font-bold text-gray-600">s</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {selectedRace.stage === 'FINAL' && (
                  <div className="lg:w-1/3 bg-gray-950 rounded-xl border border-purple-500/30 p-5">
              ...
              <div className="pt-8 border-t border-gray-800 flex justify-between items-center">
                <p className="text-gray-400 text-base">確認無誤後點擊發佈。</p>
                <button onClick={handleSubmitResults} disabled={isSubmitting} className="bg-gradient-to-r from-emerald-500 to-emerald-400 text-white px-10 py-5 rounded-2xl font-black text-2xl hover:-translate-y-1 transition-all shadow-xl">
